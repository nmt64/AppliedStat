...
- The distribution of height is normal a random.
...
confint(reg)
```
- The slope of the regression line in the 97.5% interval is 10.77
...
- Our level of certainty about the true mean is 90% in predicting that the true mean is within the interval between 168.6348 and 172.6703 assuming that the original random variable is normally distributed, and the samples are independent.
...
- Our level of certainty about the true mean is 90% in predicting that the true mean is within the interval between 163.2791 and 178.0259 assuming that the original random variable is normally distributed, and the samples are independent.
...
The arm-span measurement and height relationship is significantly different for some reasons. However, in our data, we only have height and wingspan, which is hard to conclude which is significant explanatory factor. Vitruvios' claim is still true with most of the population, but not all of them.
